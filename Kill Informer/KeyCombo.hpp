...
static std::string GetKeyName(uint8_t key) {
	static std::vector <std::pair<uint8_t, std::string>> keys = {
		{ 1 , u8"À Ã"}, //Left mouse button
		{ 2 , u8"œ Ã"}, //Right mouse button
		{ 3 , u8"VK_CANCEL"}, //Control-break processing
		{ 4 , u8"Ã Ã"}, //Middle mouse button (three-button mouse)
		{ 5 , u8"¡ Ã 1"}, //Windows 2000: X1 mouse button
		{ 6 , u8"¡ Ã 2"}, //Windows 2000: X2 mouse button
		...
		{ 89 , u8"Y(Õ)"}, //Y key
		{ 90 , u8"Z(ﬂ)"}, //Z key
		{ 91 , u8"Left Win"}, //Left Windows key (MicrosoftÆ NaturalÆ keyboard)
...
public:
	...
	void Draw(std::string label = u8"¿ÍÚË‚‡ˆËˇ", float width = 150) {
		std::string text;
		...
		if (!Edit && ImGui::TempInputIsActive(ItemID)) {
			SaveState();
			inputText = u8"Õ‡ÊÏËÚÂ Î˛·Û˛ ÍÎ‡‚Ë¯Û";
			Edit = true;
		}
	}
